...
export const PlayerBar = () => {
  ...
  const [sheetMetrics, setSheetMetrics] = useState<{ height: number; collapsedY: number; hiddenY: number }>({ height: 0, collapsedY: 0, hiddenY: 0 });
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("prosound:player-reveal", handler);
    }
    return () => {
      if (typeof window !== "undefined") {
        window.removeEventListener("prosound:player-reveal", handler);
      }
    };
  }, []);

  useEffect(() => {
    if (isMobile && currentTrack && !playerRevealed) {
      setPlayerRevealed(true);
    }
  }, [isMobile, currentTrack, playerRevealed]);
  ...
  }, [isMobile, currentTrack, sheetVisible, sheetMetrics.hiddenY, sheetMetrics.collapsedY]);
  ...
  if (isMobile) {
    ...
    return (
      <div className="fixed inset-0 z-50 pointer-events-none">
        ...
        />
        <div
          className="absolute inset-x-0 bottom-0 pointer-events-auto rounded-t-3xl bg-[var(--glass)] backdrop-blur-2xl border border-[var(--border-strong)] shadow-[0_-10px_28px_rgba(0,0,0,0.35)] overflow-visible relative"
          style={{
        ...
        >
          <div className="absolute inset-0 -z-10 bg-cover bg-center opacity-50 blur-3xl" style={{ backgroundImage: currentTrack.coverUrl ? `url(${currentTrack.coverUrl})` : "none" }} />
