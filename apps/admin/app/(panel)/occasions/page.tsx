...
export default function OccasionsPage() {
  ...
  const load = async () => { const data = await adminApi.listOccasions(); setItems(data); };
  useEffect(() => { load(); }, []);
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    await adminApi.createOccasion({ name });
    setName("");
    load();
  };
  const remove = async (id: string) => { await adminApi.deleteOccasion(id); load(); };
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-white/60">Справочник</p>
          <h1 className="text-2xl font-semibold">Поводы</h1>
        </div>
      </div>
