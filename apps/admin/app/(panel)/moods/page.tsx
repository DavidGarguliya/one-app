...
export default function MoodsPage() {
  ...
  const load = async () => { const data = await adminApi.listMoods(); setItems(data); };
  useEffect(() => { load(); }, []);
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    await adminApi.createMood({ name });
    setName("");
    load();
  };
  const remove = async (id: string) => { await adminApi.deleteMood(id); load(); };
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-white/60">Справочник</p>
          <h1 className="text-2xl font-semibold">Настроения</h1>
        </div>
      </div>
